...
const { product, error, pending, createOrUpdateProduct } = await useAdminProduct(productId);
...
const isCreating = computed(() => productId === "new");
const pageTitle = computed(() => isCreating.value ? "Crear nuevo producto" : `Editar producto: ${product.value?.name || "Cargando..."}`);
const subtitle = computed(() => isCreating.value ? "Llena el formulario para agregar un nuevo producto a tu tienda." : "Modifica los detalles del producto seleccionado.");
